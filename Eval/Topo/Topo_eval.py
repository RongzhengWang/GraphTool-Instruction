...
def calculate_topological_sort(graph):
    """Calculates the topological sort of the graph."""
    try:
        return list(nx.topological_sort(graph))
    except nx.NetworkXUnfeasible:
        return None  # 如果图中有环，拓扑排序不可行
...
def main(graph_path, ans_path):
    ...
    for i in range(total_entries):
        ...
        if api_name == "topological_sort":
            try:
                # Calculate the topological sort of the graph
                topo_sort = calculate_topological_sort(graph)
                print(f'Calculated topological sort: {topo_sort}')
            
                # Compare calculated topological sort with the provided answer
                is_correct = str(topo_sort) == str(expected_answer)
                
                if is_correct:
                    matched_entries += 1
                else:
                    # Print the details in case of a mismatch
                    print(f"Unmatched entry:\nAPI Name: {api_name}\nProvided Answer: {expected_answer}\nCalculated Topological Sort: {topo_sort}\n")
            except Exception as e:
                print(f"Error calculating topological sort: {e}")
                skipped_entries += 1
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Check the topological sort of graphs from JSON files.")
