            ...
            for item in data:
                ...
                if first_answer_data:
                    ...
                    for match in matches:
                        # 转换为整数并作为元组加入边列表
                        graph_item["edges"].append((int(match[0]), int(match[1])))
                
                # 提取每个对象的prompt键
                prompt_data = item.get("prompt", None)
                if prompt_data:
                    # 找到prompt中匹配的所有边
                    matches = edge_pattern.findall(prompt_data)
                    if matches:
                        # 将找到的边添加到prompt_list
                        for match in matches:
                            # 转换为整数并作为元组加入prompt_list
                            graph_item["prompt_list"].append((int(match[0]), int(match[1])))
...
# 调用函数并传入文件路径
file_path = "/home/data2t2/wrz/Graph Tools/1LLaMA3.1/All_type_mission_directed/once/number_of_nodes_graphCount/ans_graph.json"
