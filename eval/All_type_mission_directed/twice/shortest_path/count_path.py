...
# File paths
cycle_edge_list_path = '/home/data2t2/wrz/Graph Tools/Graph_LLaMA_test/All_type_mission_directed/twice/shortest_path/ans_graph_GLandEX.json'
ans_5ques_path = '/home/data2t2/wrz/Graph Tools/Graph_LLaMA_test/All_type_mission_directed/twice/shortest_path/ans_5ques_GLandEX.json'
...
for i in range(total_entries):
    ...
    if i < len(ans_5ques_data):
        ...
        if api_name == "shortest_path":
            try:
                # Compute the shortest path length between path_source and path_target
                shortest_path_length = nx.shortest_path_length(graph, path_source, path_target)
                
                # Compare computed shortest path length with the provided answer
                is_correct = shortest_path_length == real_answer
                
                if is_correct:
                    matched_entries += 1
                else:
                    # Print the details in case of a mismatch
                    print(f'Computed shortest path length: {shortest_path_length} Real answer: {real_answer}')
                
                # Print the results
                # print(f"Computed shortest path length from node {path_source} to node {path_target}: {shortest_path_length}")
                ...
                if 'Either source' in error_message or 'target is not in G' in error_message:
                    ...
                    print(f"Error computing shortest path length: {e}")
                    skipped_entries += 1
        else:
            skipped_entries += 1
    else:
