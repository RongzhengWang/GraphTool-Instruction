...
def main(args):
    ...
    # Read the API_name to template mapping
    ...
    for obj in tqdm(files):
        ...
        # Get the corresponding prompt2_template based on API_name
        prompt2_template = api_name_to_template.get(api_name)

        # Construct the new prompt2
        ...
        with torch.no_grad():
            outputs = model.generate(**inputs, **gen_kwargs)
            outputs = outputs[:, inputs['input_ids'].shape[1]:]
            secondanswer = tokenizer.decode(outputs[0], skip_special_tokens=True)
            print(secondanswer)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--tool_template', type=str, required=True, help='Path to the API name to template mapping JSON file.')
